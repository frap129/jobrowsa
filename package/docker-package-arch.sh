...
_image="jobrowsa-arch-packager:latest"
...
if [ ! -x "$_build_output/jobrowsa" ]; then
	echo "Jobrowsa build output not found: $_build_output/jobrowsa" >&2
	exit 1
fi
...
"$_runtime" run --rm -i \
	-u "$_user_uidgid" \
	"${_userns_args[@]}" \
	-e "JOBROWSA_VERSION=$_version" \
	-e JOBROWSA_BUILD_OUTPUT=/repo/build/src/out/Default \
	-e JOBROWSA_PACKAGE_FILES=/repo/package \
	-v "$_root_dir:/repo" \
