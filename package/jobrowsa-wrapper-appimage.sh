...
# This is the script that runs when the Jobrowsa AppImage is launched.
...
AA_PROFILE_PATH=/etc/apparmor.d/jobrowsa-appimage
...
print_apparmor_profile() {
	...
	echo
	echo 'profile jobrowsa-appimage "'"$APPIMAGE_ESC"'" flags=(default_allow) {'
	echo '  userns,'
	echo '  include if exists <local/jobrowsa-appimage>'
	echo '}'
}

if needs_apparmor_bootstrap && has_apparmor_prereqs; then
	echo "Jobrowsa has detected that your system uses AppArmor." >&2
	echo "Before Jobrowsa can run, it needs to create an AppArmor profile for itself." >&2
